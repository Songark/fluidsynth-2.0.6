                        ...
                        unsigned int ticks)
{
    ...
    while(1)
    {
        ...
        else if((track->muteflag == 1 || seeking) && (event->type == NOTE_ON /*|| event->type == NOTE_OFF*/))
        {
...
int fluid_player_set_midi_track_mute(fluid_player_t *player, int track, int flag)
{
	if (player->ntracks > track) {
		player->track[track]->muteflag = flag;
		return FLUID_OK;
	}
